export default function GaugeCard({ kaloriHariIni = 0, targetKalori }) {
  ...
  const safeKalori = Number(kaloriHariIni) || 0;
  const target = Number(targetKalori) || 0;

  const hasTarget = target > 0;
  const persen = hasTarget ? Math.min(safeKalori / target, 1) : 0;
  const sisa = hasTarget ? Math.max(target - safeKalori, 0) : 0;
  const lebih = hasTarget && safeKalori > target;
      ...
      <div className="gauge-note">
        {!hasTarget ? (
          ...
          <span style={{ color: '#ef4444' }}>
            ⚠️ Melebihi {fmt(safeKalori - target)} kcal
          </span>
        ) : (
          <>
